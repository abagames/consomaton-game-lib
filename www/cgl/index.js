...
/***/ function(module, exports, __webpack_require__) {
	...
	function init(data) {
	    ...
	    rulesDataStr = '';
	    var dataRules = data.rules != null ? data.rules : '';
	    _.forEach(dataRules.split('\n'), function (rl) {
	        ...
	        if (_.startsWith(rl, '===') || _.startsWith(rl, '---') || _.startsWith(rl, '...')) {
	            ...
	            while ((matched = regexp.exec(rl)) != null) {
	                var type = matched[1];
	                var args = matched[2] !== '' ? (matched[2].trim()).split(',') : [];
	                if (type === '/') {
	                    break;
	                }
	                addRulesWithCommand(rules, type, args);
	                commands.push({ type: type, args: args });
	                if (i++ > 16) {
	                    break;
	                }
	            }
	            currentRule = { before: [], after: [], commands: commands };
	            var ci = rl.indexOf('/');
	            if (ci >= 0) {
	                rl = rl.substr(0, ci);
	            }
	...
	function initConsole() {
	    var cls = firstConsole.split('\n');
	    cells = _.times(width, function (x) { return _.times(height, function (y) {
	        if (y >= cls.length - 1) {
	            return null;
	        }
	        var cl = cls[y + 1];
	...
	function normalizeRule(rule) {
	    ...
	    if (_.every(rule.before, function (b) { return b === '   '; }) && _.every(rule.after, function (a) { return a === '   '; })) {
	...
	function addRulesWithCommand(rules, type, args) {
	    switch (type) {
	        case 'k':
	            _.forEach(args[0], function (c) {
	                var r = { before: [c], after: [c] };
	                normalizeRule(r);
	                rules.push(r);
	            });
	            break;
	...
	function copyRulesWithCommand(rules, rule) {
	    _.forEach(rule.commands, function (c) {
	        switch (c.type) {
	            case 'd':
	                var w = c.args.length < 1 || c.args[0] !== '2' ? 4 : 2;
	                var cs = c.args.length < 2 ? null : c.args[1];
	                var ps = c.args.length < 3 ? null : c.args[2];
	                var psi = 1;
	                for (var i = 0; i < 3; i++) {
	                    if (i !== 1 && w === 2) {
	                        continue;
	                    ...
	                    var ro = rolllOffsets[i];
	                    var j = 0;
	                    for (var y = 0; y < 3; y++) {
	                        var bstr = '';
	                        var astr = '';
	                        for (var x = 0; x < 3; x++) {
	                            var b = rule.before[ro[j + 1]][ro[j]];
	                            var csi = void 0;
	                            if (cs != null && (csi = cs.indexOf(b)) >= 0) {
	                                b = cs[(csi + psi) % w];
	                            }
	                            var a = rule.after[ro[j + 1]][ro[j]];
	...
	function checkPadPressed() {
	    exports.isPadPressed = _.mapValues(keys, function () { return false; });
	    _.forOwn(keys, function (v, k) {
	        if (_.some(v, function (k) { return con.isKeyDown[k] || k == con.lastPressedKey; })) {
	            exports.isPadPressed[k] = true;
	            con.lastPressedKey = null;
	        }
	    });
	    if (con.touchPadWay != null) {
	        var touchPadWay = con.touchPadWay;
	        if (touchPadWay < -0.5 && touchPadWay > -3.5) {
	            exports.isPadPressed['^'] = true;
	        }
	        if (touchPadWay > 0.5 && touchPadWay < 3.5) {
	            exports.isPadPressed['v'] = true;
	        }
	...
	function testCond(commands) {
	    var isCondMatched = true;
	    _.forEach(commands, function (c) {
	        switch (c.type) {
	            case 'p':
	                if (!exports.isPadPressed[c.args[0]]) {
	                    isCondMatched = false;
	                    return false;
	                }
	                break;
	            case 'i':
	                var i = Number(c.args[0]);
	...
	function drawToConsole() {
	    _.times(width, function (x) { return _.times(height, function (y) { return con.texts[x][y + 1] = cells[x][y]; }); });
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";
	var _ = __webpack_require__(2);
	var fontData = __webpack_require__(5);
	var pag = __webpack_require__(6);
	var sss = __webpack_require__(7);
	var LZstring = __webpack_require__(8);
	exports.width = 16;
	exports.height = 17;
	exports.isKeyDown = _.times(256, function () { return false; });
	exports.lastPressedKey = null;
	exports.isTouching = false;
	exports.touchPadWay = null;
	var cellPixelSize = 10;
	var canvasWidth = exports.width * cellPixelSize;
	var canvasHeight = exports.height * cellPixelSize;
	var canvas;
	var context;
	var letterImages;
	var pixelArtImages;
	...
	function init(data) {
	    ...
	    exports.isLetters = _.times(exports.width, function () { return _.times(exports.height, function () { return false; }); });
	    ...
	    document.onkeydown = function (e) {
	        ...
	        exports.isKeyDown[e.keyCode] = true;
	        exports.lastPressedKey = e.keyCode;
	    };
	    document.onkeyup = function (e) {
	        exports.isKeyDown[e.keyCode] = false;
	    };
	...
	function onMouseTouchMove(x, y) {
	    if (!exports.isTouching) {
	        return;
	    }
	    var tx = x - canvas.offsetLeft - touchStartPos.x;
	    var ty = y - canvas.offsetTop - touchStartPos.y;
	    var a = Math.atan2(ty, tx);
	    var d = Math.sqrt(tx * tx + ty * ty);
	    if (d > 10) {
	        exports.touchPadWay = a / (Math.PI / 4);
	    }
	    else {
	        exports.touchPadWay = null;
	    }
	...
	function clear() {
	    _.times(exports.width, function (x) { return _.times(exports.height, function (y) {
	        ...
	        exports.isLetters[x][y] = false;
	    }); });
	}
	exports.clear = clear;
	function print(str, sx, sy) {
	    var x = sx;
	    _.forEach(str, function (c) {
	        if (x >= exports.width) {
	            return false;
	        }
	        exports.texts[x][sy] = c;
	        exports.isLetters[x][sy] = true;
	        x++;
	    });
	}
	exports.print = print;
	function drawChar(x, y) {
	    ...
	    if (c != null && c !== ' ') {
	        var cc = c.charCodeAt(0) - 32;
	        var img = exports.isLetters[x][y] ? letterImages[cc] : pixelArtImages[cc];
	        context.drawImage(img, x * cellPixelSize, y * cellPixelSize);
	    }
	}
	function createLetterImages() {
	    var imageDataUrls = JSON.parse(LZstring.decompressFromEncodedURIComponent(fontData.fontData));
	    letterImages = [];
	    pixelArtImages = [];
	    _.forEach(imageDataUrls, function (du, ci) {
	        var img = new Image();
	        img.src = du;
	        letterImages.push(img);
	        var isGenerated = false;
	        var genCvs;
	        _.forOwn(pixelArt, function (v, k) {
	            var ri;
	            if ((ri = k.indexOf(String.fromCharCode(ci + 32))) >= 0) {
	                if (!_.isArray(v)) {
	                    v = [v];
	                }
	                var pattern = v.length > 0 ? v[0].split('\n') : [];
	                pattern.pop();
	                pattern.shift();
	                var options = _.cloneDeep(pixelArtOptions);
	...
	exports.createLetterImages = createLetterImages;
	function generatePixelArt(pattern, options, ri, isRotated) {
	    ...
	    var pixels = pag.generate(pattern, options)[ri];
	    var w = pixels.length;
	    var h = pixels[0].length;
	    var o = isRotated ? 3 : 0;
	    var i = 0;
	    _.times(cvs.height, function (y) {
	        _.times(cvs.width, function (x) {
	            if (x + o < w && y + o < h) {
...
/***/ function(module, exports) {
	...
	"use strict";
	// based on the misaki font: http://www.geocities.jp/littlimi/misaki.htm
h7AmIYwGwD8C8pNADE-3GAFRjnigpzvPhoOK9EqSWmt7a7hRO6baTrgb444J+J4Dfjfx-4pwCZLMkWS8YdAKqL9gxAIUYC7hFgEwCfDKlpA7MD-GlUeTaA78oNFAFEBwzmQYRLFRSVNOUnhTZpXnULC7Vu7MDzhlwq5DcNYROAIAtDcDMMUoLVx3h-ARwgZNFShp9x5TS4DOjhT----JcDQBrgrQWpUDMwMCDn4okO+f9iVjPRAKNQUAlYYp2Za2kGMKC2UbwquX7DJVfo6VbYufGolpSMoFlJcFHawCVsfadruhiZar4zg34cQHRBkTXB9GlvFgKICakvI6ADC-gK2HIDyAwFlncppZKPgahKA2iZ4JYFs4Ata5nSwRtcqxVerDa9yiUvYsJUygRIqpcgC4FZhMBTkkkjVTrAioJ88kwYWUNkCiCkc9s8AFAMWsdCfFYkHACMGkFKUgUk6F44KVeMuXVqPVZfOtdPB9VzkCV74w4K2vpIdrT0pyD7jzTCB1sGFfgdQZoPmjNh7QxI2UNY2oocA-AzwBhaaSU5nxQ+aATEGEHDkE18xrq9dXsPK5brX+IgAGXirEXNrD1--LO4AKZMBmIa82QKWA4BoxLA9bDUGkD4I4xxAAAv6qZn2WyAnwYAZYRK13av0tNessVXwpm2saCtPYQGcGOOQJZM5LwBTGoCgbBkzg2gTQGM1ODVQxR4CFHvwD8BOrYRWw7Td9vdUsb7xZEtwIrygD0JGaWfNVCwA1AkBFZlAEENJBjLvCmAUsGwE+GZR6AnwiAMPOIG0SyhAgNnTQNcCwBkL92lgDgN+HBSryasIAxlpNtRY-bN1f2ubQ2sGXFbm1kingHSWAWlgSNscAxuvA1Cyo8o0+FwAJKojuA1F5kOEIaWMpcqDCo4EbpUEMAQAMAiAIVSuommHyaZqS-TQVosBFaIgW4GIHEASBJAUg6QTINkDyCFBnd40kVZ3AAC6QAA';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateIntervalBase = 12;
	var stateTicks = 0;
	var updateTicks = 0;
	var isAutomatonBegun = false;
	var wasPadPressed = false;
	var padUnpressedTicks = 0;
	var score = 0;
	var title;
	...
	function init(data) {
	    ...
	    if (data.dev) {
	        debug.initSeedUi(seed, function (s) {
	            pag.setSeed(s);
	            con.createLetterImages();
	            sss.reset();
	            sss.setSeed(s);
	            if (exports.isInGame && isBgmEnabled) {
	                sss.playBgm();
	            }
	    ...
	    if (updateTicks <= 0) {
	        ...
	        if (!exports.isInGame) {
	            if (stateTicks < 180) {
	                ...
	                var x = Math.floor(8 - title.length / 2);
	                if (x < 0) {
	                    x = 0;
	                }
	                con.print(title, x, 6);
	        ...
	        con.update();
	        updateTicks = Math.floor(exports.isInGame ?
	            updateIntervalBase / (Math.sqrt(stateTicks / 1000 + 1)) :
	            updateIntervalBase * 1.7);
	        exports.gameTicks++;
	    }
	    if (!exports.isInGame && stateTicks > 60 && isAnyKeyPressed()) {
	        begin();
	    }
	    stateTicks++;
	...
	function begin() {
	    exports.isInGame = isAutomatonBegun = true;
	    stateTicks = 0;
...
/***/ function(module, exports) {
	...
	"use strict";
	function initSeedUi(seed, onSeedChanged) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	function save(_data) {
	    ...
	    var encDataStr = LZString.compressToEncodedURIComponent(JSON.stringify(_data));
	    var url = baseUrl + "?v=" + version + "&d=" + encDataStr;
	    try {
	...
	exports.save = save;
	function saveModified(modifiedData) {
